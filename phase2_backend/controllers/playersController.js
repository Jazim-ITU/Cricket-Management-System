...
const createPlayer = async (req, res, next) => {
  ...
  try {
    const [result] = await pool.execute(
      `INSERT INTO Players (first_name, last_name, date_of_birth, role, batting_style, bowling_style, nationality, team_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [first_name, last_name, date_of_birth || null, role, batting_style || null, bowling_style || null, nationality || null, team_id || null]
    );
...
const updatePlayer = async (req, res, next) => {
  ...
  try {
    ...
    await pool.execute(
      `UPDATE Players
       SET first_name = ?, last_name = ?, date_of_birth = ?, role = ?,
